...
class Bridge(Infra):
    ...
    def __init__(self, unique_id, model, length=0,
        ...
        self.condition = str(condition).strip().upper() if condition is not None else 'UNKNOWN'

        if self.length > 200:
            self.delay_time = self.model.random.triangular(1, 4, 2) * 60.0
...
class Sink(Infra):
    ...
    def remove(self, vehicle):
        ...
        self.model.completed_trips.append({
            'vehicle_id': vehicle.unique_id,
            'generated_at_step': vehicle.generated_at_step,
            'removed_at_step': vehicle.removed_at_step,
            'driving_time_min': vehicle.removed_at_step - vehicle.generated_at_step,
            'delay_time_min': vehicle.total_delay_time
        })
...
class Vehicle(Agent):
    ...
    def drive_to_next(self, distance):
        ...
        if isinstance(next_infra, Sink):
            ...
            if self.waiting_time > 0:
                self.total_delay_time += self.waiting_time
